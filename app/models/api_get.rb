class ApiGet
  ...
  def get_api
    ...
    get = { train: { name: name_train_delay } }
  ...
  def get_api_with_position(lat, lng)
    ...
    restaurant_gurunavi["rest"].first(count).each do |restaurant|
      get_restaurant << { name: restaurant["name"], category: restaurant["category"], url: restaurant["url"], image_url: restaurant["shop_image1"], address: restaurant["address"] }
    end
